...
const IngredientScanner = () => {
  ...
  const sendText = async (text) => {
    ...
    try {
      // Corrected Port to 5001 and Route to /analyze
  ...
  const processResponse = (data) => {
    ...
    const extracted_text = data?.extracted_text;
    ...
    if (!analysis && data) {
      setResult({
        ...
        verdict_text: data.verdict_text || "Analysis complete",
        better_alternative: data.better_alternative || "",
        dosha_summary: data.dosha_summary || {},
    ...
    if (Array.isArray(analysis.identified_ingredients)) {
      const vata = analysis.identified_ingredients
        .filter((i) => i.dosha_effect?.balances?.includes("Vata"))
        .map((i) => i.matched_name);
      const pitta = analysis.identified_ingredients
        .filter((i) => i.dosha_effect?.balances?.includes("Pitta"))
        .map((i) => i.matched_name);
      const kapha = analysis.identified_ingredients
        .filter((i) => i.dosha_effect?.balances?.includes("Kapha"))
        .map((i) => i.matched_name);

      let better_alternative = "";
      if (analysis.identified_ingredients?.length > 0) {
        better_alternative =
    ...
    const harmful_ingredients = (analysis.identified_ingredients || [])
      .filter((i) => i.safety_rating === "avoid")
      .map((i) => ({
        name: i.matched_name,
        effect:
          i.toxicity_notes ||
    ...
    try {
      ...
      setResult((prev) => ({
        ...
        verdict_text:
          ...
          "Connection error. Ensure backend is on port 5001.",
      }));
    } finally {
      setLoading(false);
    }
