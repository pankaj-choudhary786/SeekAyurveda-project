...
import { Book, Send, Activity, Smile, Frown, Meh } from "lucide-react";
...
const HealthTracker = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FFE4BB] flex flex-col items-center pt-15 pb-8">
      <header className="relative flex items-center justify-between bg-[#286459] h-14 w-[94%] max-w-7xl rounded-full px-5 my-8 shadow-sm z-20 shrink-0">
      ...
      <section className="w-full flex justify-center grow px-4">
          ...
          <div className="w-full rounded-xl bg-white/40 backdrop-blur-md border border-white/50 p-6 flex flex-col shadow-sm min-h-[350px]">
            ...
            <div className="flex-1 flex flex-col justify-end relative pb-6 px-2 min-h-[200px]">
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-20 z-0 pb-6">
                {[...Array(5)].map((_, i) => (
                  <div key={i} className="border-b border-black w-full h-full"></div>
                ))}
              </div>

              <svg viewBox="0 0 100 60" className="w-full h-full overflow-visible z-10" preserveAspectRatio="none">
                <defs>
            ...
            <div className="mt-4 p-3 bg-[#FFE4BB]/50 rounded-lg text-xs text-[#286459] text-center">
              Your average mood is <strong>7.1/10</strong> this week. Keep it up!
            </div>
          </div>

          <div className="w-full rounded-xl bg-white/40 backdrop-blur-md border border-white/50 p-6 flex flex-col shadow-sm h-full">
            <h2 className="text-[#286459] font-bold text-lg mb-1">Daily Check-in</h2>
            <p className="text-xs text-gray-500 mb-6">How are you feeling right now?</p>

            <div className="flex-1 flex flex-col gap-6">
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                  <Frown className={`transition-colors ${moodScore < 4 ? 'text-red-500' : 'text-gray-400'}`} size={24}/>
                  <span className="text-3xl font-bold text-[#286459]">{moodScore}</span>
                  <Smile className={`transition-colors ${moodScore > 7 ? 'text-green-600' : 'text-gray-400'}`} size={24}/>
                </div>
                <input
                  type="range"
                  min="1"
              ...
              <div>
                <label className="text-xs font-semibold text-[#286459] block mb-2">Feeling...</label>
                <div className="flex flex-wrap gap-2">
                  {["Anxious", "Calm", "Tired", "Energetic", "Focused"].map((tag) => (
                    <button key={tag} className="px-3 py-1 rounded-full text-xs border border-[#286459]/30 hover:bg-[#286459] hover:text-white transition-colors text-[#286459]">
                      {tag}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex-1 min-h-[100px]">
                <label className="text-xs font-semibold text-[#286459] block mb-2">Notes</label>
                <textarea
                  value={note}
